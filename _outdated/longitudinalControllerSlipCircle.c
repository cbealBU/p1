...
#define S_FUNCTION_NAME  longitudinalControllerSlipCircle
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    ...
    if ( fabs(alphaFront)>alphaFrontLimit || fabs(FBdeltaAlphaLimited)>0 || ( pow(alphaRearRatio,2)+pow(kappaRearRatio,2) )>1 ) 
    // either front wheel or all wheels are sliding 
    {
        ...
        {
            if ( fabs(alphaRear)<alphaRearLimit )   // Kappa exceed the slip circle, but alpha still within the limit
            {
                deltaKappaRear=fabs(kappaRear)-sqrt(1-pow(alphaRearRatio,2));       
                if (kappaRear>0){
                    FxCommand=gainRearKappaSlip*deltaKappaRear;
                }
                else // kappa less or equal to zero
                {
                    FxCommand=-gainRearKappaSlip*deltaKappaRear;
                }
            }
            else // alpha rear go beyond the limit
            {
                deltaKappaRear=fabs(kappaRear);
                 if (kappaRear>0){  // brake too much
                    FxCommand=gainRearKappaSlip*deltaKappaRear+gainRearAlphaSlip*(fabs(alphaRear)-alphaRearLimit);
                }
                 else // kappa less or equal to zero   // accerelate too much
                {
                    FxCommand=-gainRearKappaSlip*deltaKappaRear-gainRearAlphaSlip*(fabs(alphaRear)-alphaRearLimit);
                }
            }
    ...
    {
        if (segType == 0){  // straight, trust the feedforward and do not brake or accerelate more
            FxCommand=0;
        }      
        else // not in a straight line
        {
            deltaKappaRear=fabs( sqrt(1-pow(alphaRearRatio,2))-fabs(kappaRear) );
            // either let off brake more or accerelate more
            if ( fabs( alphaFrontLimit-fabs(alphaFront) )> frontScalingLimit ){ // no scaling
                FxCommand=gainRearNoneSlip*deltaKappaRear;
            }
            else{   // smooth out the command to avoid any jump when front tires start to saturate
                FxCommand=(fabs( alphaFrontLimit-fabs(alphaFront) )/frontScalingLimit )*gainRearNoneSlip*deltaKappaRear;
            }
        }
    }
