...
#define S_FUNCTION_NAME Brush
#define S_FUNCTION_LEVEL 2
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          5
/* Input Port  0 */
#define IN_PORT_0_NAME      sigma
...
{
    const real_T   *sigma  = (const real_T*) ssGetInputPortSignal(S,0);
    const real_T   *Fz  = (const real_T*) ssGetInputPortSignal(S,1);
    const real_T   *alpha  = (const real_T*) ssGetInputPortSignal(S,2);
    const real_T   *mu_p  = (const real_T*) ssGetInputPortSignal(S,3);
    const real_T   *mu_s  = (const real_T*) ssGetInputPortSignal(S,4);
    real_T        *Fx  = (real_T *)ssGetOutputPortRealSignal(S,0);
    real_T        *Fy  = (real_T *)ssGetOutputPortRealSignal(S,1);
    real_T        *Tsat  = (real_T *)ssGetOutputPortRealSignal(S,2);
    const int_T   p_width0  = mxGetNumberOfElements(PARAM_DEF0(S));
    const int_T   p_width1  = mxGetNumberOfElements(PARAM_DEF1(S));
    const int_T   p_width2  = mxGetNumberOfElements(PARAM_DEF2(S));
    const real_T  *Ca  = mxGetData(PARAM_DEF0(S));
    const real_T  *Cs  = mxGetData(PARAM_DEF1(S));
    const real_T  *lc  = mxGetData(PARAM_DEF2(S));
...
Vs=sqrt(pow(sigma[0],2)+pow(tan(alpha[0]),2));
if (Vs>1.0) Vs = 1.0;
if (Vs<0.0) Vs = 0.0;
mu=mu_p[0]*(1.0-Vs*0.2);

if ( sigma[0] >= 0 ){
    lambda = sqrt(sigma[0]*sigma[0]+ pow(K_alpha/K_sigma,2)*tan(alpha[0])*tan(alpha[0]));
    xi = 1.0-K_sigma/(3.0*mu_s[0]*W)*lambda/(1.0-sigma[0]);
    ...
    if (xi > 0){    
        Fx[0] = -K_sigma*sigma[0]/(1.0-sigma[0])*pow(xi,2)-6.0*mu*W*Cos_theta*(1.0/6.0-1.0/2.0*xi*xi+1.0/3.0*xi*xi*xi);
        Fy[0] = -K_alpha*tan(alpha[0])/(1.0-sigma[0])*pow(xi,2)-6.0*mu*W*Sin_theta*(1.0/6.0-1.0/2.0*pow(xi,2)+1.0/3.0*pow(xi,3));
        Tsat[0] = l*K_alpha*tan(alpha[0])/(2.0*(1.0-sigma[0]))*pow(xi,2)*(1.0-4.0/3.0*xi)
         -3.0/2.0*l*mu*W*Sin_theta*pow(xi,2)*pow(1.0-xi,2)
         +2.0*l*K_sigma*sigma[0]*tan(alpha[0])/(3.0*pow(1.0-sigma[0],2))*pow(xi,3)
         +3.0*l*pow(mu*W,2)*Sin_theta*Cos_theta/(5.0*K_alpha)*(1.0-10.0*pow(xi,3)+15.0*pow(xi,4)-6.0*pow(xi,5));
    }
    else{
        Fx[0] = -mu*W*Cos_theta;
        Fy[0] = -mu*W*Sin_theta;
        Tsat[0]=3.0*l*pow(mu*W,2)*Sin_theta*Cos_theta/(5.0*K_alpha);
    }
}

else{
     lambda = sqrt( pow(sigma[0],2)+pow(K_alpha/K_sigma,2)*pow(1+sigma[0],2)*pow(tan(alpha[0]),2) );
     xi = 1.0-K_sigma/(3.0*mu_s[0]*W)*lambda;
    if (lambda == 0 && sigma == 0){ 
        Cos_theta = 1.0;
        Sin_theta = 0.0;
    }
        ...
        Cos_theta = sigma[0]/lambda;
        Sin_theta = K_alpha*(1.0+sigma[0])*tan(alpha[0])/(K_sigma*lambda);
    }
    if (xi > 0){
        Fx[0] = -K_sigma*sigma[0]*pow(xi,2)-6.0*mu*W*Cos_theta*(1.0/6.0-1.0/2.0*pow(xi,2)+1.0/3.0*pow(xi,3));
        Fy[0] = -K_alpha*(1.0+sigma[0])*pow(xi,2)*tan(alpha[0])-6.0*mu*W*Sin_theta*(1.0/6.0-1.0/2.0*pow(xi,2)+1.0/3.0*pow(xi,3));
        Tsat[0] = l/2.0*K_alpha*(1.0+sigma[0])*tan(alpha[0])*pow(xi,2)*(1.0-4.0/3.0*xi)
        -3.0/2.0*l*mu*W*Sin_theta*pow(xi,2)*pow(1.0-xi,2)
        +2.0/3.0*l*K_sigma*(1.0+sigma[0])*sigma[0]*tan(alpha[0])*pow(xi,3)
        +3.0*l*pow(mu*W,2)*Sin_theta*Cos_theta/(5.0*K_alpha)*(1.0-10.0*pow(xi,3)+15.0*pow(xi,4)-6.0*pow(xi,5) );
    }
     else{
        Fx[0] = -mu*W*Cos_theta;
