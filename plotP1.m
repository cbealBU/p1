...
offset_delta_LF = 0.1*pi/180;
offset_delta_RF = -0.1*pi/180;
...
%% Calculate a simple brush tire model
param.Ca = 45000; param.mu = 2.8; param.mu_s = 1.0; % inside wheel 
...
thetay = Calpha./(3*param.mu.*FzModel);
...
FyModel(satInds) = -sign(alphaModel(satInds)).*mus.*FzModel(satInds);
MzModel(satInds) = 0;
...
num=wc*2*pi*[1 -1];
den=[1 wc*2*pi*Ts-1];
tInds = t > 0 & t < 90; %ibegin = 70*500; iend = 430*500;
...
offset_Fy_LF = -220; offset_Fy_RF = 375;
offset_Mz_LF = 40; offset_Mz_RF = -85;
plot(t,[(-(Wheel_Forces(:,11) - offset_Fy_LF)./(Wheel_Forces(:,3)-offset_Mz_LF) - tml).*(abs(Wheel_Forces(:,3)) > 100) (-(Wheel_Forces(:,12)- offset_Fy_RF)./(Wheel_Forces(:,4) - offset_Mz_RF) - tmr).*(abs(Wheel_Forces(:,4)) > 100)]);
linkHands(4) = gca;
xlabel('Time (s)')
ylabel('Total Trail (m)')
legend('LF','RF')
...
dirInds = logical(tInds .* GPS(:,10) > 0.5);
hold off
plot3(alphafl(dirInds,1)*180/pi,Wheel_Forces(dirInds,5),Wheel_Forces(dirInds,11),'b.');
hold on
plot3(alphafr(dirInds,1)*180/pi,Wheel_Forces(dirInds,6),Wheel_Forces(dirInds,12),'.','color',[0 0.5 0]);
xlabel('Slip Angle (deg)')
ylabel('Vertical Load (N)')
